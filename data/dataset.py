...
class SetDataset:
    def __init__(self, data_file, batch_size, transform, return_gt = False):
        with open(data_file, 'r') as f:
            self.meta = json.load(f)
        ...
        for cl in self.cl_list:
            ...
            sub_dataset = SubDataset(self.sub_meta[cl], cl, transform=transform, return_gt=return_gt)
...
class SubDataset:
    ...
    def __getitem__(self, i):
        ...
        if not self.return_gt:
            ...
            gt = transforms.Compose(self.transform.transforms[:3])(
                Image.open(image_path).convert('RGB'))  # transform = ['Resize','CenterCrop', 'ToTensor', 'Normalize']
            img = transforms.Compose(self.transform.transforms[3:])(gt)
            target = self.target_transform(self.cl)
...
class WeightedEpisodicBatchSampler(object):
    def __init__(self, n_classes, n_way, n_episodes):
        self.n_classes = n_classes
        self.n_way = n_way
        self.n_episodes = n_episodes
        self.w_classes = torch.ones(self.n_classes).exp()
    def __len__(self):
        return self.n_episodes

    def __iter__(self):
        for i in range(self.n_episodes):
            key = torch.rand(self.n_classes)**(1/self.w_classes.data)
            order = key.argsort(descending=False)
            yield order[:self.n_way]
